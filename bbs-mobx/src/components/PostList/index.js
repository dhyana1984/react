...
export default class PostList extends Component{
  ...
  //保存新建的帖子
  handleSave = data =>{
    this.props.postsStore.createPost(data).then(()=>{
  ...
  render(){
    ...
    return (
      <div className="postList">
        <div>
          <h2>帖子列表</h2>
          {authStore.userId ?(<button onClick={this.handleNewPost}>发帖</button>): null}
        </div>
        {uiStore.addDialogOpen?(<PostEditor onSave={this.handleSave} onCancel={this.handleCancel}/>):null}
        <PostView posts={postsStore.posts} />
      </div>
    )
  }
}